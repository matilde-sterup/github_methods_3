...
#***ATTENCIONE*** #Model failed to converge with max|grad| = 0.00299939 (tol = 0.002, component 1)
...
print(summary(gh))
```
Accuracy increases significantly faster with objective evidence for PAS 3 than for PAS 2.
...
print(summary(gh))
```
Accuracy does not increase significantly faster with objective evidence for PAS 4 than for PAS 3.
...
contrast.vector <- rbind(c(0, 0, 0, 0, 0, 1, 0, 0), c(0, 0, 0, 0, 0, 0, -1, 1))
rownames(contrast.vector) <- c("PAS 2-PAS 1", "PAS 4-PAS 3")
gh <- glht(m7, contrast.vector) 
...
We see in this comparison that the estimates are different. Next we want to test this difference:

```{r}
print(summary(gh, test = Ftest()))
#Maybe??

plot(gh, xlab= "Estimates") #or dis??
```
COMMENTS:

The 95% confidence intervals for the differences in estimates between PAS2-PAS1 () and PAS4-PAS3 () show no overlap (see plot). 
