...
plt.xlabel('x')
plt.ylabel('y')
plt.title('Plot Exercise 1.2iii')
...
```{python}
#100 samples of len(x)
samples = [y_true + np.random.normal(loc = 0, scale = 5, size = len(x)) for i in range(100)] #scale = sigma
```
...
#linear
linear_fits = [linear.fit(x_reshape,y) for y in samples]

#quadratic
quadratic = PolynomialFeatures(degree = 2)
X_quadratic = quadratic.fit_transform(x_reshape)
quadratic_fits = [regressor.fit]
