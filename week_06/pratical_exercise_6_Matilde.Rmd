...
library(reticulate)
print(conda_list())
print(rp <- Sys.getenv('RETICULATE_PYTHON'))
```
...
#Scatterplot
plt.figure() # create new figure
plt.plot(x, y, 'bo') # plot a blue scatter
plt.plot(x,yhat, 'r-') # line (red)
plt.xlabel('x')
plt.ylabel('y')
...
#Find beta hat based on this X
betas = np.linalg.inv(X.T@X)@X.T@y
print(betas) #these are the means
...
#Find beta hats: 
#1) the difference between the mean of y1 and the mean of y2 
#2) the mean of $y_2$
